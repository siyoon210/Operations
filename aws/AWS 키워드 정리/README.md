...
### AWS CloudWatch
- AWS가 제공하는 자원과 애플리케이션에 대한 모니터링 및 관리 서비스
- 지표 : 언제 어떤 항목의 값이 무엇인지 기록한 값. ex) CPU사용량, 네트워크 전송량, Auto Scaling 그룹이 관리하는 인스턴스의 수
...
- 결국 EC2, Auto Scaling 그룹, CloudWatch와 같은 서비스를 사용하지만 자동화되는 것이 포인트
- 단점으로는 자동화 되다보니 설정이나 버그를 파악하는대 유연하지 못하다는 점이 있다.
- 일반적인 서비스 환경이라면 충분히 사용해봐도 괜찮다.

### AWS SQS (Simple Queue Service)
- 메시지 대기열 서비스로 한 서비스에서 다른 서비스로 메시지를 넘길때 주로 사용된다.
